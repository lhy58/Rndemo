...
const ITEM_WIDTH = SIZES.width * 0.6; // 0.8
      ...
      <Animated.View style={{ transform: [{ translateY }] }}>
        {data.map((item, index) => (
          <View key={item.menuId} style={styles.itemContainer}>
            <Text style={{ ...FONTS.h2 }}
              onPress={() => {
                navigation.navigate('animateSharedDetail', {
                  item: data[index]
                })
              }}>{item.name}</Text>
            <View>
...
const AnimateShared = ({ navigation }) => {
            ...
            renderItem={({ item, index }) => {
              ...
              return (
                ...
                >
                  ...
                  <SharedElement id={`item.${item.menuId}.image`}>
                    <Image
                      source={item.photo}
                      style={{
                        width: ITEM_WIDTH,
